...
function run_stress-system() {
	...
	if [ "${OS_VIRT}" == "docker" ]; then
		...
		sudo apptainer instance start -C --bind /scratch2:/scratch2 "${STRESS_CONTAINER_DIR}"/stress.sif stress ${OTHER_OPTIONS}-l "${LOAD}" -s "${STRESSORS}" --cpu-load-types "${LOAD_TYPES}" -c "${CURRENT_CORES}" -t 4m >> "${LOG_FILE}" 2>&1
	fi
	sleep 240
