...
# Lista global para mantener los contenedores en ejecución
running_containers = []
...
if __name__ == "__main__":
    ...
    containers_receiver = ContainersReceiver(logger, target_node)
    containers_receiver.create_thread()
    ...
    influxdb_client = InfluxDBClient(url=influxdb_url, token=influxdb_config['influxdb_token'], org=influxdb_config['influxdb_org'])
    ...
    try:
        while True:
            ...
            try:
                for container in containers_receiver.get_running_containers():
                    ...
                    with open(output_file[cont_pid], 'r') as file:
                        ...
                        if len(bulk_data) > 0:
                            ...
                            try:
                                influxdb_client.write_api(write_options=SYNCHRONOUS).write(bucket=influxdb_bucket, record=target_metrics)
        ...
        logger.error(f"Unexpected error taking out of the loop: {str(e)}")
    finally:
        influxdb_client.close()
        containers_receiver.stop_thread()